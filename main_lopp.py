...
class MyGrid(Widget):
    ...
    def update(self, showlast):
        ...
        if len(kuupaevad) == 1:
            ...
            button = Button(text= buttontext1, pos=(x, y), size=(20, 20), background_color=vastavus[kuupaevad[1]][1]) 
            button.bind(on_press = lambda *args, i=1: self.on_buttonpress(vastavus[kuupaevad[i]][0]))
            self.ids.w_canvas.add_widget(button)

            x = Window.width/2 - Window.width/5
            button = Button(text= buttontext2, pos=(x, y), size=(20, 20), background_color=vastavus[kuupaevad[2]][1]) 
